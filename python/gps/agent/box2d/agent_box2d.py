...
class AgentBox2D(Agent):
    ...
    def sample(self, policy, condition, verbose=False, save=True, noisy=True):
        ...
        """
        self._worlds[condition].run()   #reset the world to it's initial condition
        self._worlds[condition].reset_world()  #stores all the value of the initial state
        b2d_X = self._worlds[condition].get_state()    #receives the state of the point mass in this case EEP and EEPV
